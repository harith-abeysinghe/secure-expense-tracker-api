...
public class BudgetService {
    ...
    public BudgetService(BudgetRepository budgets, UserService users, CategoryService categories) {
    ...
    public BudgetResponse upsert(UUID userId, UUID categoryId, int year, int month, BigDecimal amount) {
        ...
        var budget = budgets.findByUserIdAndCategoryIdAndYearAndMonth(userId, categoryId, year, month)
                .map(existing -> { existing.setAmount(amount); return existing; })
    ...
    public static void validateMonth(int year, int month) {
        ...
        try { YearMonth.of(year, month); }
        catch (DateTimeException exception) { throw new IllegalArgumentException("month must be between 1 and 12"); }
    }
